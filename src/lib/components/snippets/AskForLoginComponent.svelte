<script lang="ts">
  import { FaSolidArrowLeft, FaSolidX } from 'svelte-icons-pack/fa';
  ...
  const finalAction = async (): Promise<void> => {
...
<div class="flex h-full w-full items-center justify-center" in:scale|global>
  ...
  >
    ...
    <button class="absolute top-4 right-4" aria-label="close" onclick={modal.close} type="button">
      <Icon size="20" src={FaSolidX} />
    </button>
    {#if state === 'choosing'}
      <div class="flex w-full flex-col gap-4 pt-8">
        {#if allowGuest}
          {@render setStateButton(choices.guest.name, choices.guest.action, choices.guest.reverseColor)}
        {/if}
        {@render setStateButton(choices.signIn.name, choices.signIn.action, choices.signIn.reverseColor)}
        {@render setStateButton(choices.signUp.name, choices.signUp.action, choices.signUp.reverseColor)}
      </div>
    {:else if state === 'sign in'}
      <SignIn actionAfterAuthentication={finalAction} showAuthSwitchMessage={false} />
    {:else if state === 'sign up'}
      <SignUp actionAfterAuthentication={finalAction} showAuthSwitchMessage={false} />
    {/if}
  </div>
</div>
