<script lang="ts" module>
  ...
  export { askForLoginModalSnippet };
...
{#snippet askForLoginModalSnippet(modal: ModalInstance<Params>, { allowGuest, action }: Params)}
        ...
        <div class="flex w-full flex-col gap-4 pt-8">
          {#if allowGuest}
            {@render setStateButton(
              choices.guest.name,
              choices.guest.getAction(modal, action),
              choices.guest.reverseColor,
            )}
          {/if}
          {@render setStateButton(
            choices.signIn.name,
            choices.signIn.getAction(modal, action),
            choices.signIn.reverseColor,
          )}
          {@render setStateButton(
            choices.signUp.name,
            choices.signUp.getAction(modal, action),
            choices.signUp.reverseColor,
          )}
        </div>
      {:else if state === 'sign in'}
        <SignIn actionAfterAuthentication={getFinalAction(modal, action)} showAuthSwitchMessage={false} />
